...
export const createFhevmInstance = async (parameters: {
  ...
  // Only the Sepolia relayer is supported in non-mock mode.
  // Guard early to avoid sending malformed requests to the relayer from unsupported networks (e.g., 31337).
  if (chainId !== 11155111) {
    throwFhevmError(
      "UNSUPPORTED_RELAYER_CHAIN",
      `Relayer is only supported on Sepolia (chainId=11155111). Current chainId=${chainId}. Switch to Sepolia or run against an FHEVM Hardhat node.`,
    );
  }
