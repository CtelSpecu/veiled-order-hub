...
const CONTRACT_NAMES = ["FHECounter", "VeiledOrder"];

// <root>/packages/fhevm-hardhat-template
// Resolve deployments directory - try multiple paths
const candidates = [
  path.resolve("../deployments"),
  path.resolve("../../veiled-order-hub/deployments"),
];
...
if (!deploymentsDir) {
  ...
  deploymentsDir = path.join(path.resolve(".."), "deployments");
}
...
function readDeployment(chainName, chainId, contractName, optional) {
  if (!deploymentsDir) {
    if (!optional) {
      console.error(`${line}Deployments directory not found.${line}`);
      process.exit(1);
    }
    return undefined;
  }
